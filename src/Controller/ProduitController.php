...
class ProduitController extends AbstractController
{
    ...
    /**
     * @Route("/paiement/{id_produit}", name="paiement")
     */
    public function payer(Request $request, $id_produit): Response
    {

        $produit = $this->getDoctrine()->getManager()->getRepository(Produit::class)->find($id_produit);
        $intent = $this->stripe->paymentIntents->create([
            'amount' => $produit->getPrix(),
            'currency' => 'usd',
        ]);


      /*  $successUrl = $this->generateUrl('app_payement_save', [
            'clientId' => 1,
            'modePayment' => $abonnement->getModePaiementAb(),
            'prixCourse' => $abonnement->getTypeAb(),
        ]);*/
        ...
        $link = $this->stripe->checkout->sessions->create([
            ...
            'line_items' => [
                [
                    'price_data' => [
                        'currency' => 'usd',
                        'product_data' => [
                            'name' => 'product '.$produit->nom_produit(),
                        ],
            ...
            'mode' => 'payment',
            'success_url' => 'http://127.0.0.1:8000/displayfrontcategorie',
            'cancel_url' => 'http://127.0.0.1:8000/displayfrontcategorie'
   ...
   /**
    ...
    {
        $Demande = new Demande();
        $Produit = $this->getDoctrine()->getRepository(Produit::class)->findOneBy(['idProduit' => $request->query->get('id')]);
        $user = $this->getUser();
        ...
        $this->addFlash('success', 'La demande a été envoyée avec succès !');


        return $this->redirectToRoute('displayfront', ['id' => $Produit->getIdCategorie()->getId()]);
    }
...
/**
     * @Route("/searchedjsonprod", name="searchedjsonprod")
     */
